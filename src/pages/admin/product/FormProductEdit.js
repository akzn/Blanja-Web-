...
const FormProductEdit = (props) => {
  ...
  const getCategory = async () => {
    await axios
      ...
      .then((res) => {
        ...
        setCtg(categories[1].id_categories)
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (cekColor.length === 0) {
        alert("Please select at least one color."); // Display an alert message
    ...
    if (cekSize.length === 0) {
        alert("Please select at least one size."); // Display an alert message
        return; // Stop further execution
    }
  ...
  // Redirect to product list if save success
  if(addP === true) {
